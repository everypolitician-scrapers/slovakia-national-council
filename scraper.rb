...
def fill_independents(mems)
  ...
  all = sorted.each_cons(2).map do |one, two|
    gap = ind.merge(start_date: one[:end_date] + 1,
                    end_date: two[:start_date] - 1)
    gap = nil if gap[:end_date] - gap[:start_date] < 2
    [one, gap, two]
  end.flatten.compact.uniq.map do |r|
...
groups = noko_q('organizations', where: %({"classification":"parliamentary group"})).map do |group|
  data = {
...
people = noko_q('people', max_results: 50,
...
people.each do |person|
  ...
  term_mems = person.xpath('memberships[organization[classification[text()="chamber"]]]').map do |gm|
  ...
  end.reject { |tm| tm[:id].to_i == 1 } # no faction information available for term 1

  group_mems = fill_independents(person.xpath('memberships[organization[classification[text()="parliamentary group"]]]').map do |gm|
    {
      name: gm.xpath('organization/name').text,
      id: gm.xpath('.//identifiers[scheme[text()="nrsr.sk"]]/identifier').text,
  ...
  combine(term: term_mems, faction_id: group_mems).each do |mem|
    ...
    ScraperWiki.save_sqlite(%i[id term faction_id start_date], data.reject { |_k, v| v.to_s.empty? })
  end
end
